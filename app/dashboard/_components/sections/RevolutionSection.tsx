      ...
      <div className="revolution-grid">
        {cards.map((c) => (
          <div key={c.title} style={{ background: c.gradient, borderRadius: "16px", border: "1px solid rgba(255,255,255,0.08)", padding: "28px", textAlign: "left", transition: "all 0.3s ease", cursor: "default", overflow: "hidden", position: "relative" }}
            ...
            <span style={{ display: "inline-block", padding: "5px 12px", borderRadius: "20px", backgroundColor: `${c.tagColor}18`, color: c.tagColor, fontSize: "11px", fontWeight: 600, marginBottom: "18px" }}>{c.tag}</span>
            <h3 style={{ color: "#ffffff", fontSize: "20px", fontWeight: 600, margin: "0 0 10px" }}>{c.title}</h3>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "13px", margin: 0, lineHeight: 1.7 }}>{c.desc}</p>
