...
import { Bot, User, Image as ImageIcon, Paperclip, Send, Copy, Check } from "lucide-react";
...
const markdownComponents = {
  ...
  code: ({ children, className, ...props }: any) => {
    ...
    return <code style={{ backgroundColor: "rgba(255,255,255,0.1)", padding: "2px 6px", borderRadius: "4px", fontSize: "12px", fontFamily: "monospace", color: "#e2e8f0" }} {...props}>{children}</code>;
  },
                        ...
                        <div style={{ display: "flex", gap: "6px", marginBottom: msg.content ? "8px" : "0", flexWrap: "wrap" }}>
                          {msg.attachments.filter(a => a.type !== "image").map((att, i) => (
                            <span key={i} style={{ background: "rgba(0,0,0,0.2)", padding: "4px 8px", borderRadius: "6px", fontSize: "12px", display: "flex", alignItems: "center", gap: "6px" }}>
                              <Paperclip size={12} style={{ color: "rgba(255,255,255,0.7)" }} /> <span style={{ maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{att.name}</span>
                            </span>
                          ))}
                        </div>
                      )}
            ...
            <div style={{ display: "flex", gap: "8px", padding: "0 4px", flexWrap: "wrap" }}>
              {imageFile && imagePreviewUrl && (
                <div style={{ position: "relative", display: "inline-block" }}>
                  ...
                  <button onClick={() => setImageFile(null)} style={{ position: "absolute", top: "-6px", right: "-6px", width: "20px", height: "20px", borderRadius: "50%", background: "rgba(0,0,0,0.7)", border: "1.5px solid rgba(255,255,255,0.3)", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", lineHeight: 1, padding: 0 }}>&times;</button>
                </div>
              )}
              {docFile && (
