...
export default function CompanionSection() {
  return (
    <section id="features" style={{ padding: "80px 20px", textAlign: "center", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      `}</style>
      <h2 style={{ color: "#ffffff", fontSize: "clamp(22px, 5vw, 34px)", fontWeight: 700, margin: "0 0 8px" }}>The Only AI Companion You Need to Succeed</h2>
      ...
      <div className="companion-grid">
        {features.map((f) => (
          <div key={f.title} style={{ backgroundColor: "#111118", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.06)", padding: "28px", textAlign: "left", transition: "all 0.3s ease", cursor: "default" }}
            ...
            <div style={{ width: "44px", height: "44px", borderRadius: "12px", backgroundColor: "rgba(79,70,229,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "22px", marginBottom: "16px" }}>{f.icon}</div>
            <h3 style={{ color: "#ffffff", fontSize: "17px", fontWeight: 600, margin: "0 0 8px" }}>{f.title}</h3>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "13px", margin: 0, lineHeight: 1.6 }}>{f.desc}</p>
          </div>
        ))}
      </div>
    </section>
